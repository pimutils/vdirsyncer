...
impl Response {
    pub fn new() -> Self {
        Response { href: None, etag: None, has_collection_tag: false, mimetype: None }
    }
}
...
impl<T: BufRead> ListingParser<T> {
    pub fn new(mut reader: quick_xml::Reader<T>) -> Self {
        ...
        ListingParser { reader, ns_buf: vec![] }
    }
    ...
    fn next_response(&mut self) -> Fallible<Option<Response>> {
        ...
        loop {
            match self.reader.read_namespaced_event(&mut buf, &mut self.ns_buf)? {
                (ns, Event::Start(ref e)) => {
                    match (state, ns, e.local_name()) {
                        ...
                        (State::Response, Some(b"DAV:"), b"getcontenttype") => state = State::ContentType,
